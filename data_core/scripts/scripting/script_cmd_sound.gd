...
func execute(context: ScriptExecutionContext) -> bool:
	...
	match sound_type:
		...
		SoundType.MUSIC:
			# Usar el bus de música global
			if AudioManager.has_method("play_music"):
				AudioManager.play_music(sound_path, volume_db, loop)
			return true
			
		SoundType.VOICE:
			# Similar a SFX pero con prioridad diferente
			audio_player = AudioStreamPlayer.new()
			audio_player.stream = stream
			audio_player.volume_db = volume_db
			audio_player.bus = "Voice"  ## Asumiendo que tienes un bus Voice
			context.npc.add_child(audio_player)
			audio_player.play()
...
func _on_sound_finished(context: ScriptExecutionContext) -> void:
	...
	if context.npc and context.npc.has_node("ScriptRunner"):
		var runner = context.npc.get_node("ScriptRunner")
		if runner and runner.has_method("on_async_complete"):
			runner.on_async_complete()

func get_display_text() -> String:
	var type_names = ["SFX", "Música", "Voz"]
