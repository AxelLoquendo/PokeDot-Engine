...
var variables: Dictionary = {}  ## Variables temporales del script

func _init(npc_node: Node2D = null, player_node: Node2D = null, map_node: Node = null):
	npc = npc_node
	player = player_node
	map = map_node
...
func move_npc_to_tile(tile_pos: Vector2i, wait: bool = true) -> void:
	if not npc:
		return
	var controller = get_npc_controller()
	if controller and controller.has_method("mover_a_casilla"):
		controller.mover_a_casilla(tile_pos)
		if wait:
			is_waiting = true
...
func move_player_to_tile(tile_pos: Vector2i, wait: bool = true) -> void:
	if not player:
		return
	var controller = player.get("character_controller") if player.has_property("character_controller") else player
	if controller and controller.has_method("mover_a_casilla"):
		controller.mover_a_casilla(tile_pos)
		if wait:
			is_waiting = true
