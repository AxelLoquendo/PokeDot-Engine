...
@export_group("Editor")
@export var color_borde: Color = Color(0.0, 1.0, 1.0, 1.0)
@export var tile_size: int = 16
@export var mostrar_limite: bool = true:
	set(value):
		mostrar_limite = value
		queue_redraw()
...
@export var show_location_name: bool = false:
	set(value):
		show_location_name = value
		if not Engine.is_editor_hint():
			show_location_name_changed.emit(value)

signal usar_nubes_cambiado(estado: bool)
@export var usar_nubes: bool = true:
	set(value):
		usar_nubes = value
		usar_nubes_cambiado.emit(value)
...
## Compatibilidad con mapas antiguos (antes del sistema de MapScriptEntry).
...
func conectar_actualizaciones() -> void:
	var conexiones: Array[MapConnection] = [north_map, south_map, east_map, west_map]
	for conexion: MapConnection in conexiones:
		if conexion:
			conexion.connection_changed.connect(queue_redraw)
...
func _draw() -> void:
	...
	var ancho: float = float(map_size.x * tile_size)
	var alto: float = float(map_size.y * tile_size)

	draw_line(Vector2.ZERO, Vector2(ancho, 0.0), color_borde, 2.0)
	draw_line(Vector2(ancho, 0.0), Vector2(ancho, alto), color_borde, 2.0)
	draw_line(Vector2(ancho, alto), Vector2(0.0, alto), color_borde, 2.0)
	draw_line(Vector2(0.0, alto), Vector2.ZERO, color_borde, 2.0)
...
func dibujar_conexion_norte() -> void:
	if north_map == null:
		return
	dibujar_conexion(ConnectionDirection.NORTH, north_map, Color.RED)


func dibujar_conexion_sur() -> void:
	if south_map == null:
		return
	dibujar_conexion(ConnectionDirection.SOUTH, south_map, Color.GREEN)


func dibujar_conexion_este() -> void:
	if east_map == null:
		return
	dibujar_conexion(ConnectionDirection.EAST, east_map, Color.YELLOW)


func dibujar_conexion_oeste() -> void:
	if west_map == null:
		return
	dibujar_conexion(ConnectionDirection.WEST, west_map, Color.ORANGE)


func dibujar_conexion(
	...
	var posicion: Vector2 = obtener_posicion_conexion(direccion, conexion)
	...
	draw_rect(Rect2(posicion, tamaño), color, false, 2.0)

	var fuente: Font = ThemeDB.fallback_font
	if fuente:
		draw_string(
			fuente,
			posicion + Vector2(6.0, 18.0),
			mapa.map_name,
...
func dibujar_preview_mapa(mapa: MapAttributes, posicion: Vector2) -> void:
	...
	for hijo: Node in tilesets.get_children():
		if hijo is TileMapLayer:
			...
			dibujar_tilemap(layer, posicion)


func dibujar_tilemap(layer: TileMapLayer, offset: Vector2) -> void:
	...
	for celda: Vector2i in layer.get_used_cells():
		...
		draw_texture_rect_region(
			atlas,
			Rect2(offset + Vector2(celda * tile_size), Vector2(tile_size, tile_size)),
			region
		)
...
func actualizar_borde_visual() -> void:
	var borde: TileMapLayer = get_node_or_null("Behaviours/Borde") as TileMapLayer
	if borde == null:
		return
	...
	for x: int in range(-grosor, map_size.x + grosor):
		for y: int in range(-grosor, 0):
			crear_tile_borde(borde, Vector2i(x, y))
...
func crear_tile_borde(layer: TileMapLayer, pos: Vector2i) -> void:
	...
	layer.set_cell(pos, border_source_id, atlas)


# Métodos usados por MapManager
@warning_ignore("unused_parameter")
func obtener_metatile(tile: Vector2i) -> int:
	# Por ahora devolvemos 0 para que compile.
	return 0


@warning_ignore("unused_parameter")
func obtener_border_metatile(tile: Vector2i) -> int:
	# Por ahora devolvemos 0 para que compile.
	return 0
