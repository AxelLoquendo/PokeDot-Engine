...
func caer(delta: float) -> void:
	...
	if (
		position.y > area.end.y + 50.0
		or position.x < area.position.x - 50.0
		or position.x > area.end.x + 50.0
	):
...
func terminar() -> void:
	...
	if weather != null:
		...
		var clima: RainWeather = weather

		clima.gota_terminada(self)
...
func reiniciar() -> void:
	...
	position = Vector2(
		...
		randf_range(
			area.position.y - 300.0,
			area.position.y - 50.0
		)
	)
