...
@export_group("Cadencia en el sitio")
## Duration multipliers apply only to in-place animation actions. They never
## alter the duration of normal tile movement.
@export_range(0.5, 4.0, 0.05) var walk_in_place_duration_multiplier: float = 1.60
@export_range(0.5, 4.0, 0.05) var jog_in_place_duration_multiplier: float = 1.30
@export_range(0.5, 4.0, 0.05) var run_in_place_duration_multiplier: float = 1.00
@export_range(0.5, 4.0, 0.05) var slowly_in_place_duration_multiplier: float = 2.10

func obtener_multiplicador_cadencia_en_sitio(mode: String) -> float:
	match mode:
		"jog": return jog_in_place_duration_multiplier
		"run": return run_in_place_duration_multiplier
		"slowly": return slowly_in_place_duration_multiplier
		_: return walk_in_place_duration_multiplier
...
## Explicit visual-step boundary shared by player, NPC, and in-place actions.
## It toggles first/second authority before rendering exactly one step.
func iniciar_paso_animacion() -> void:
...
# Compatibility aliases for player scripts that used private helper names.
func _mirar_hacia(direccion: Vector2) -> void:
	mirar_hacia(direccion)


func _obtener_direccion_deseada() -> Vector2:
	return obtener_direccion_deseada()
